CREATE DATABASE  IF NOT EXISTS `inn` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `inn`;
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `calculate_food_total_charges` AFTER INSERT ON `food_order_detail` FOR EACH ROW BEGIN
    ...
    remaining_amount = total_booking_charges - deposit
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `calculate_total_booking_charges_for_room` AFTER INSERT ON `room_charges` FOR EACH ROW BEGIN
	...
	UPDATE booking_charges
    SET deposit = total_room_charges * 0.3
    WHERE booking_id = NEW.booking_id;
    
    UPDATE booking_charges
    SET remaining_amount = total_booking_charges - deposit
    WHERE booking_id = NEW.booking_id;
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_booking`(IN `p_room_no` VARCHAR(5), IN `p_customer_name` VARCHAR(50), IN `p_ph_no` VARCHAR(25), IN `p_check_in` DATETIME, IN `p_stay_night` INT, IN `p_stay_hour` INT, IN `p_payment_method` VARCHAR(30))
BEGIN
    ...
    IF v_customer_id IS NULL THEN
        ...
        VALUES (p_customer_name, p_ph_no, 'pending', 'pending');
	...
	SELECT deposit INTO v_deposit FROM booking_charges WHERE booking_id = v_booking_id;

	INSERT INTO payment(booking_id, amount, payment_date, payment_method) VALUES(v_booking_id, v_deposit, CURRENT_TIME, p_payment_method);
...
/*!50003 SET sql_mode              = 'NO_AUTO_VALUE_ON_ZERO' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `add_checkIn`(IN `p_customer_name` VARCHAR(100), IN `p_ph_no` VARCHAR(15), IN `p_id_card` VARCHAR(50), IN `p_email` VARCHAR(100), IN `p_room_no` VARCHAR(20), IN `p_check_in` DATETIME, IN `p_stay_night` INT, IN `p_stay_hour` INT, IN `p_payment_method` VARCHAR(30))
BEGIN
    DECLARE v_customer_id INT;
    DECLARE v_check_out DATETIME;
    DECLARE last_accepted_date DATETIME;
    ...
    WHERE customer_id = v_customer_id 
      ...
      AND check_out = v_check_out;
        ...
        UPDATE room SET room_status = 'Unavailable' WHERE room_no = p_room_no; 
        UPDATE booking_room_detail SET booking_status = 'Arrived' WHERE booking_id = v_booking_id;

	SELECT deposit INTO v_deposit FROM booking_charges WHERE booking_id = v_booking_id;

	INSERT INTO payment(booking_id, amount, payment_date, payment_method) VALUES(v_booking_id, v_deposit, CURRENT_TIME, p_payment_method);

    ELSEIF v_room_status = 'Unavailable' OR v_room_status = 'Booked' THEN
        -- Check for overlapping bookings for the same room
        SELECT COUNT(*) INTO overlap_exists
        FROM booking 
        JOIN booking_room_detail ON booking.booking_id = booking_room_detail.booking_id 
...
BEGIN
    ...
    SET room_charges = room_charges * (SELECT deposit_rate FROM deposit)
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `search_available_rooms_within_dates` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `search_available_rooms_within_dates`(

    IN check_in DATETIME,
    IN check_out DATETIME
)
BEGIN
    SELECT r.room_no, r.room_type_id, r.floor, r.room_status, rt.description
    FROM room r
...
BEGIN
    SELECT room.room_no, room_type.description, room.floor
    FROM room
    JOIN room_type ON room.room_type_id = room_type.room_type_id
    LEFT JOIN booking_room_detail brd 
        ON room.room_no = brd.room_no
    LEFT JOIN booking b 
        ON brd.booking_id = b.booking_id
        AND brd.booking_status IN ('Booked', 'Arrived') -- Only consider 'Booked' or 'Arrived' as unavailable
        AND (b.check_in < end_date AND b.check_out > start_date) -- Check if booking overlaps with requested dates
    WHERE brd.room_no IS NULL -- No booking for the room during the specified dates
    AND room.room_status = 'Available'; -- Only available rooms
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
-- Dump completed on 2024-11-14  9:49:26
